...
use consts::{DEFAULT_TT_SIZE, MAX_PV, MAX_THREADS};
...
impl OptionsMap {
    ...
    fn multi_pv() -> Box<UCIOption> {
        let mutator: fn(i32) -> Option<OptionWork> = |x: i32| Some(OptionWork::MultiPV(x as usize));
        Box::new(UCISpin {
            option_name: "MultiPV",
            default: 1 as i32,
            min: 1,
